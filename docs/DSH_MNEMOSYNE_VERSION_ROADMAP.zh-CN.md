...
> 状态：✅ `v0.1.0` MVP 实现与发布门禁完成，待发布提交签收
> 日期：2026-08-24
> 当前插件版本：`0.1.0`
> 当前待迁移 DSH 基线：本机 CLI `0.1.1-rc.2`，公开 SDK `0.1.1-rc.2`
> 唯一产品目标：管理 DSH 的短期记忆与长期记忆
...
4. 保持旧记忆、OKF Generation 和 Tool 协议可迁移；
...
| 版本 | 完整目标 | 相对上一版的提升 |
|---|---|---|
| `v0.1.0` | 完整记忆 MVP | 从 synthetic 只读实验变成真正可用的短期/长期记忆插件 |
| `v0.2.0` | 记忆质量增强 | 从“能够记住和找回”提升到“记得更准、找得更准、错误可纠正” |
| `v0.3.0` | 管理与规模化 | 从单项目 Tool 管理提升到可靠运维、可视管理和安全跨项目复用 |
...
`v0.1.0` 能形成、保存和读取记忆，但长期使用后会出现重复、碎片、过时、错误候选以及基础文本检索召回不足。

`v0.2.0` 集中提升记忆本身和检索结果的质量。

### 5.2 新增能力

- Revision：修订产生新版本，旧版本保留；
- Freeze/Restore：错误或暂时不适用的记忆退出默认索引但不删除；
- History：查看来源、修订和管理记录；
- 自动提取后的精确去重与近似重叠提示；
- 多条短期记忆整合为更完整的长期记忆；
- OKF 关系：`related_to`、`depends_on`、`supersedes`、`derived_from`；
- 确定性别名扩展、中文 n-gram、结构字段和关系辅助检索；
- 短期/长期、Session/Project、最近使用和成功复用的明确排序规则；
- Search Receipt 记录 Generation、候选世界和页面引用，不保存模型思考；
- 固定 Fixture 对比 v0.1.0 的召回、误披露和 Token 成本。

### 5.3 强制提升 Gate

相对 v0.1.0 必须证明：

- 困难换措辞 Fixture 召回提高或至少不下降；
- Frozen Leakage 为 0；
- 错误候选率不增加；
- 重复长期记忆数量下降；
- 用户可以修订、冻结和恢复错误记忆；
...
完成标志：`memory_quality_ready`。

## 六、v0.3.0：管理与规模化

### 6.1 解决 v0.2.0 的限制

`v0.2.0` 的记忆质量已经可控，但主要依靠 Tool 操作，且只支持单项目；长期运行还会遇到损坏、残留锁、空间增长和跨项目复用需求。
...
| `v0.2.0` | `v0.3.0` | 单项目 Tool 管理 → 可诊断、可视管理、安全跨项目复用 |
